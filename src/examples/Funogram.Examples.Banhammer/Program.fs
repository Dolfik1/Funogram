...
let processBanCommand (msg: Message) (from: User) banlist ctx =
  async {
    match! Api.getChatMember msg.Chat.Id from.Id |> api ctx.Config with
    ...
    | Result.Ok (ChatMember.Owner _) ->
      ...
      | Some { From = Some replyMessageFrom } ->
        ...
        match! Api.banChatMember msg.Chat.Id replyMessageFrom.Id |> api ctx.Config with
        | Result.Ok _ ->
      ...
      | _ ->
        Api.sendMessageReply
          ...
          msg.MessageId
        |> api ctx.Config |> Async.Ignore |> Async.Start
  ...
  async {
    ...
    if bannedUsers.Length = 0 then
      ...
      let! result = 
        ...
        |> Seq.map (fun banned ->
          async {
            match! Api.banChatMember msg.Chat.Id banned.Id |> api ctx.Config with
      ...
      Api.sendMessage msg.Chat.Id text |> api ctx.Config |> Async.Ignore |> Async.Start
