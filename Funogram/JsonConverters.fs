...
type DuConverter() = 
    ...
    let getCasesTypes (cases: UnionCaseInfo[]) = 
        cases |> Array.map (fun f -> (f, f.GetFields().[0].PropertyType))
    ...
    let getTypeProps (t: Type) (serializer: JsonSerializer) =
        ...
        if serializer.ContractResolver :? DefaultContractResolver then
            ...
            joc.Properties |> Seq.map (
                fun f -> (f.PropertyName, 
                            (props 
                            |> Seq.findBack (
                                fun h -> h.Name = f.UnderlyingName))))
        else
            props |> Seq.map (fun f -> (f.Name, f))
    ...
    let rec isJsonObjectAndTypeEquals (t: Type) (jObject: JObject) (serializer: JsonSerializer) = 
        ...
        let isJsonEquals() = jsonProps 
                            |> Seq.forall (
                                fun f -> not (sourceProps |> Seq.forall(fun (k, _) -> k <> f.Key)))

        let isTypeEquals() = sourceProps 
                                |> Seq.where (fun (_, v) -> isRequiredType (v.GetType()))
                                |> Seq.forall (fun (k, _) -> not (jsonProps |> Seq.forall(fun v -> v.Key <> k)))
             

        if not (isJsonEquals()) || not (isTypeEquals()) then
            false
        else
    ...
    override x.ReadJson(reader, t, existingValue, serializer) = 
        ...
        if not (isNull reader.ValueType) then // if type known
            match casesTypes |> Array.tryFind (fun (_, f) -> f = reader.ValueType) with
            | Some (u, t) -> FSharpValue.MakeUnion(u, [|serializer.Deserialize(reader, t)|])
            | None -> null
        else
            let jObject = JObject.Load(reader)
            let tp = casesTypes |> Seq.tryFind(fun (_, t) -> isJsonObjectAndTypeEquals t jObject serializer)
            match tp with
            | Some (u, t) -> FSharpValue.MakeUnion(u, [|jObject.ToObject(t, serializer)|]) 
            | None -> null